...
if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
        ...
        case 'creator':
               ...
               buttons = [{buttonId: '!sourcecode',buttonText:{displayText:'SC BOT'},type:1},{buttonId:'!sewabot',buttonText:{displayText:'SEWA BOT'},type:1}]
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6283866844016`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break*/
               case 'spamcall':{
   ...
   case 'devil': 
               ...
               break  
   case 'putra': 
               sendKontak(from, `6283893530404`, `Putra Store`, 'Sibukk!!')
               break  
   case 'dhani': 
   ...
   case 'adi': 
    ...
    case 'patner':
               list = []
               listmenu = [`bara`,`ryzer`,`hans`,`budi`,`devil`,`putra`,`dhani`,`xinz`,`kunz`,`adi`,]
               listmenuu = [`Bara Bot`,`Ryzer Bot`,`Hans Bot`,`The Sad Boy 0.1`,`Devil Bot`,`Putra Bot`,`Bot Dhani`,`Xinz Bot`,`Kunz Bot`,`Adi Official`,]
               nombor = 1
               startnum = 0
               ...
               listmsg(from, `Halo Kak Silahkan Pilih List Patner ${botName} Di Bawah Ya Kak!\nJangan Lupa Sewa Kak Xixi`, `  `, list)
               break                                                     
...
putra.sendMessage(from, loli, MessageType.audio, {quoted: freply, mimetype: 'audio/mp4', ptt:true})
                    putra.sendMessage(from, { contentText: `${menu}`, footerText: 'Jika Button Di Bawah Tidak Muncul, silahkan Ketik  !command Atau !allmenu', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'LIST MESSAGE' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🛒 STORE' }, type: 1 },{ buttonId: `!patner`, buttonText: { displayText: '👤 PATNER' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6283866844016@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
┗━▹
  `
sendButMessage(from, wibu, `created Putra Store`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
textny = 'https://i.postimg.cc/ZYFF1dJm/IMG-20220101-WA0001.jpg'
...
┃▹  ${prefix}jadwalsholat
┗━▹`
putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'By Putra Ganzz', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'LIST MESSAGE' }, type: 1 },{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
...
`
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow Tiktok @putragans644`},type:1}]
              imageMsg = (await putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Putra Store', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow Tiktok @putragans644`,buttons,headerType:4}
...
 _${prefix}next_`
            var imgs = await putra.prepareMessage('0@c.us', gambar, image, { thumbnail: tamnel })
            var imgCatalog = imgs.message.imageMessage
            var ctlg = await putra.prepareMessageFromContent(from, {
            "productMessage": {
            "product": {
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await Putra.prepareMessage(from, inifile, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'㋡𝘗𝘶𝘵𝘳𝘢 𝘎𝘢𝘯𝘴', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Putra.relayWAMessage(prep)
...
break
              ...
              putra.relayWAMessage(prep)
                ...
                reply (mess.wait)
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                    ini_txt = args.join(" ")
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/cup?apikey=beta&text=LoLHuman/${command}?apikey=${setting.lolkey}&text=${ini_txt}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Putra StoreT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow Tiktok @putragans644`,buttons,headerType:4}
              prep = await putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              putra.relayWAMessage(prep)
        ...
        putra.sendMessage(from, gambar, image, { quoted: troli })
                    })
                    break
                case 'tiktok':
                case 'arcade8bit':
...
`
 ...
 Putra Store︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
`
putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'By Putra Ganzz', buttons: [{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🏷️ BACK TO MENU' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
   //addfiturbokep
                case 'bokep1':				 
...
qute = fs.readFileSync('./media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: ftrol, caption: '*SEMOGA DI BERI HIDAYAH*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
...
`
 ...
 Putra Store︎`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`allmenu`,`runtime`,`status`,`groupmenu`,`photoxy`,`makermenu`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`gcmikel`,`sewabot`,`info`,`store`,`patner`,]
               listmenuu = [`All Menu Bot`,`Runtime`,`Status Bot`,`Menu Group`,`Photo Oky`,`Image Maker`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`18+ Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`,`Thanks To`,`Store Putra Bot`,`Patner Putra Bot`,]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
               ...
               case 'store':
               list = []
               listmenu = [`share`,`logo`,`edit`,`murid`,`sewabot`,`diamondml`,`diamondff`,`ucpubg`,]
               listmenuu = [`Jasa Share`,`Logo Jb`,`Jasa Edit`,`Murid Bot`,`Sewa Bot`,`Diamonds Mobille legends`,`Diamonds Free Fire`,`UC Pubg`,]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
               ...
               listmsg(from, `Halo Kak Silahkan Pilih Menu Store ${botName} Di Bawah Ya Kak!\nJangan Lupa Sewa Kak Xixi`, `  `, list)
               break
case 'murid':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: ftoko})
textny = 'https://i.postimg.cc/ZYFF1dJm/IMG-20220101-WA0001.jpg'
...
◁━━━━◈✙◈━━━━▷
Open Murid Bot By Putra Store

Jenis Bot yg kami Melayani :


Bot Termux :

Di Beri Command Lengkap
Di Beri Script
Di Ajarin Sampe Bisa

Bot Web replit & Wibu Soft :

Di Ajarin Buat Akun Replit
Di Ajarin Deploy Di replit
Di Ajarin Sampe Bisa
...
━━━━━━━ •♬• ━━━━━━━
`
putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'By Putra Ganzz', buttons: [{ buttonId: `!payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🏷️ BACK TO MENU' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
case 'edit':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: ftoko})
textny = 'https://i.postimg.cc/ZYFF1dJm/IMG-20220101-WA0001.jpg'
...
MINAT?
 CHAT NOMOR DI BAWAH INI!
 
http://wa.me/6283866844016
━━━━━━━ •♬• ━━━━━━━
`
putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'By Putra Ganzz', buttons: [{ buttonId: `!payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🏷️ BACK TO MENU' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
...
◁━━━━◈✙◈━━━━▷
List Harga Diamond Mobille Legends
3 diamonds💎	1.500.
12 diamonds💎  3.325.
28 diamonds💎  7.600.
36 diamonds💎  9.500.
...
296 diamonds💎	 76.000.
370 diamonds💎	 95.000.
568 diamonds💎	142.500.
875 diamonds💎	 218.500.
1159 diamonds💎	285.000.
2010 diamonds💎	475.000.
4830 diamonds💎	1.140.000.
6050 diamonds💎	1.425.000.
━━━━━━━ •♬• ━━━━━━━
...
140 diamonds💎 	20.000.
355 diamonds💎	 50.000.
720 diamonds💎 	 100.000.
1450 diamonds💎	200.000.
2180 diamonds💎	300.000.
3640 diamonds💎	500.000.
7290 diamonds💎	1.000.000.
36500 diamonds💎	5.000.000.
73100 diamonds💎	10.000.000
━━━━━━━ •♬• ━━━━━━━
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated Putra Store︎`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
`
  buttons = [{buttonId: `${prefix}buypremium`,buttonText:{displayText: `Premium`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
              imageMsg = (await putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Putra Store︎ ', imageMessage: imageMsg,
...
━━━━━━━ •♬• ━━━━━━━
`
putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'By Putra Ganzz', buttons: [{ buttonId: `!menu`, buttonText: { displayText: 'BACK TO MENU' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              putra.sendMessage('6283866844016@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              putra.sendMessage('6283866844016@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT RAVA DONE')
              putra.sendMessage('6283866844016@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
