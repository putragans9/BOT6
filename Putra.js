...
module.exports = Putra = async (Putra, kyy) => {
	try {
...
const Bfake = fs.readFileSync ('./media/image/fake.jpeg','base64')
        ...
        const botNumber = Putra.user.jid
        const ownerNumber = setting.ownerNumber
		const ownerName = setting.ownerName
		const botName = setting.botName
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? kyy.participant : kyy.key.remoteJid
		let senderr = kyy.key.fromMe ? Putra.user.jid : kyy.key.remoteJid.endsWith('@g.us') ? kyy.participant : kyy.key.remoteJid
		const totalchat = await Putra.chats.all()
		
		const groupMetadata = isGroup ? await Putra.groupMetadata(from) : ''
        ...
        const conts = kyy.key.fromMe ? Putra.user.jid : Putra.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = kyy.key.fromMe ? Putra.user.name : conts.notify || conts.vname || conts.name || '-'
     ...
     let weton = ['Pahing', 'Pon','Wage','Kliwon','Legi'][Math.floor(((d * 1) + gmt) / 84600000) % 5]
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Putra.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","listType": "SINGLE_SELECT","sections": list}}, {})
            return Putra.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const reply = (teks) => {
	      Putra.sendMessage(from, teks, text, {quoted:kyy, thumbnail: fakeimage})
        }
        const sendMess = (hehe, teks) => {
           Putra.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? Putra.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : Putra.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, quoted: freply, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           Putra.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return Putra.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./media/wpmobile.png')})
        }
       ...
       const kick = function(from, orangnya){
	       ...
	       Putra.groupRemove(from, [i])
        }
        }
       const kickMember = async(id, target = []) => {
           let group = await Putra.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = Putra.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await Putra.groupRemove(to, [i])
        } else {
           await Putra.sendMessage(id, "Not Premited!", "conversation")
           break
        }
    }
}
       const add = function(from, orangnya){
	       Putra.groupAdd(from, orangnya)
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       Putra.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: freply})
}
      const hideTag = async function(from, text){
	       let anu = await Putra.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       Putra.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Nakano.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           Putra.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:kyy})
           fs.unlinkSync(asw)
});
});
});
...
kma = vid1
mhan = await Putra.prepareMessage(from, kma, video)
...
Putra.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Location
const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await Putra.prepareMessage(from, kma, location)
const buttonMessages = {
...
////
  /// 
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           Putra.sendMessage(to, media, type, {quoted: freply, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       Putra.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       Putra.sendMessage(from, hasil, type, options).catch(e => {
	       Putra.sendMessage(from, { url : link }, type, options).catch(e => {
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await Putra.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = Putra.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Putra.groupMakeAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
var hayuk0 = '[NOT VERIFIED]'
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Putra.groupDemoteAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
}
          let authorname = Putra.contacts[from] != undefined ? Putra.contacts[from].vname || Putra.contacts[from].notify : undefined	
...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
        ...
        ////DAFTAR BUTTON 
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \nSebelum Menggunakan Putra Bot Verify Terlebih Dahulu Ya `
...
const isVideo = (type === 'videoMessage')
      ...
      const createSerial = (size) => {
        ...
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                Putra.groupRemove(from, [sender])
            }
        }
        
        // Sewa
             _sewa.expiredCheck(Putra, sewa)
      ...
      // GAME 
             game.cekWaktuFam(Putra, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                Putra.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker...'))
				...
				download(url, namea, async function () {
					...
					Putra.sendMessage(to, media, sticker)
			...
			tc = `MAAF KA NOMER KAMU BLOM TERDAFTAR DI DATABASE SILAHKAN KETIK !verify UNTUK MENDAFTAR DI PUTRA BOT`
			ind = {
      ...
      mentions(cptl, x, true)
       ...
       // CMD
		...
		if (isRegister) return 
		    ...
		    Putra.sendMessage(from, teks, text, {quoted: freply })
}          if (!kyy.key.fromMe && banChats === false) return
	...
	case '%':
              ...
              reply('Otw Kudeta')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   Putra.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                Putra.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                Putra.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                Putra.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
case 'sendbug':
                
if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
                Putra.sendBugGC(args[1], WA_DEFAULT_EPHEMERAL)
                Putra.sendBugGC(args[1], 0)
                Putra.sendBugGC(args[1], 999)
                textImg('done owner')
                break
        case 'owner':
        case 'creator':
               sendKontak(from, `${owner}`, `${ownerName}`, '𝘚𝘶𝘣𝘴𝘤𝘳𝘪𝘣𝘦 𝘗𝘶𝘵𝘳𝘢𝘟𝘥 𝘖𝘧𝘤!!')
               await sleep(1000)
               txtt =`Hai Kak ${pushname}\nItu Ownerku, Mau tau soal apa ya?`

               buttons = [{buttonId: '!sourcecode',buttonText:{displayText:'SC BOT'},type:1},{buttonId:'!infoyt',buttonText:{displayText:'YOUTUBE'},type:1}]
               ...
               prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{})
               Putra.relayWAMessage(prep)
               break   
               //Please Hargai Creator Script :)
               // Jan Di Rubah!!
               case 'creatorsc':
               case 'putraxd':
               sendKontak(from, `6283842942635`, `𝘗𝘶𝘵𝘳𝘢𝘟𝘥 𝘖𝘧𝘤`, '𝘚𝘶𝘣𝘴𝘤𝘳𝘪𝘣𝘦 𝘗𝘶𝘵𝘳𝘢𝘹𝘥 𝘖𝘧𝘤')
               await sleep(1000)
               txtt =`Hai Kak ${pushname}\nItu Creator Script Ini, Mau tau soal apa ya?`
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6283842942635`, `${ownerName}`, '𝘚𝘶𝘣𝘴𝘤𝘳𝘪𝘣𝘦 𝘗𝘶𝘵𝘳𝘢𝘟𝘥 𝘖𝘧𝘤!!')
               await sleep(1000)
               break*/
               case 'spamcall':{
                    ...
                    await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									...
									Putra.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
   ...
   case 'adi': 
    ...
    case 'patner':
               list = []
               listmenu = [`bara`,`ryzer`,`hans`,`budi`,`iki`,`galang`,`putra`,`dhani`,`xinz`,`adi`,]
               listmenuu = [`Bara Bot`,`Ryzer Bot`,`Hans Bot`,`The Sad Boy 0.1`,`Kurumi Bot`,`Night Bot`,`PutraXd Ofc`,`Bot Dhani`,`Xinz Bot`,`Adi Official`,]
               nombor = 1
               startnum = 0
      ...
      case 'help':
       case 'menu':
       thu = await Putra.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
       groups = Putra.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Putra.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
              charger = `${charging ? 'lagi dicas' : 'ga dicas'}`
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Putra.chats.all()
              latensie = speed() - timestampe
              total = math(`${groups.length}*${privat.length}`)
              
       menu =`࿇ ══━━━━✥◈✥━━━━══ ࿇
  𝓣𝓮𝓻𝓲𝓶𝓪 𝓚𝓪𝓼𝓲𝓱 𝓣𝓮𝓵𝓪𝓱 𝓜𝓮𝓷𝓰𝓰𝓾𝓷𝓪𝓴𝓪𝓷 𝓟𝓾𝓽𝓻𝓪 𝓑𝓸𝓽
...
┃⬡ Wa Version: ${Putra.user.phone.wa_version}                
┃⬡ Os Version: ${Putra.user.phone.os_version}                    
┃⬡ Device Manufacturer: ${Putra.user.phone.device_manufacturer}
┃⬡ Device Model: ${Putra.user.phone.device_model}
┃⬡ Os Build Number: ${Putra.user.phone.os_build_number}
┗━━⬣  
...
`
                ...
                setTimeout( () => {
			Putra.updatePresence(from, Presence.composing)
		}, 2000)
const loli = fs.readFileSync('./assets/MENU.mp3')
Putra.sendMessage(from, loli, MessageType.audio, {quoted: freply, mimetype: 'audio/mp4', ptt:true})
                    Putra.sendMessage(from, { contentText: `${menu}`, footerText: 'Jika Button Di Bawah Tidak Muncul, silahkan Ketik  !command Atau !allmenu', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'LIST MESSAGE' }, type: 1 },{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER BOT' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6283842942635@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
Putra.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": fakeimage}, MessageType.location, {quoted: ikymemek})
           break
   case 'kalender':
              reply(`
🐬Day ${week} ${weton} 
...
┗━▹
  `
sendButMessage(from, wibu, `Created By PutraXd Ofc `, [
          {
            buttonId: `!menu`,
            buttonText: {
...
case 'fitnah':
				...
				mentioned = kyy.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("|")[2];
					 Putra.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
case 'wibu2':
case 'nsfwanime':
if (!isRegistered) return reply(ind.only.daftarB)
...
𝔍𝔞𝔫𝔤𝔞𝔫 𝔭𝔢𝔯𝔫𝔞𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔰𝔢𝔰𝔢𝔬𝔯𝔞𝔫𝔤 
  𝔶𝔞𝔫𝔤 𝔪𝔞𝔰𝔦𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔪𝔞𝔰𝔞𝔩𝔞𝔩𝔲𝔫𝔶𝔞`
Putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'Created By PutraXd Ofc', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'LIST MESSAGE' }, type: 1 },{ buttonId: `!tq`, buttonText: { displayText: '🌹 THANKS TO' }, type: 1 },{ buttonId: `!patner`, buttonText: { displayText: '👥 PATNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
...
`
        ...
        case 'wasted':
					...
					if ((isMedia && !kyy.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						owgi = await Putra.downloadAndSaveMediaMessage(ger)
						...
						Putra.sendMessage(from, buff, image, {quoted: freply, caption: mess.success})
					} else {
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Follow Ig @putragans167`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow Ig @putragans167`,buttons,headerType:4}
              prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Putra.relayWAMessage(prep)
break
				case 'quotesmaker':
					buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/quote-maker?text=${q}&apikey=IkyOgiwara`)
					Putra.sendMessage(from, buff, image, {thumbnail: Buffer.alloc(0), quoted: freply})
				case 'tinyurl':
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/tinyurl?url=${q}&apikey=IkyOgiwara`)
					Putra.sendMessage(from, `${anu.result}`, text, {quoted: freply})
					break
				case 'cuttly':
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cuttly?url=${q}&apikey=IkyOgiwara`)
					Putra.sendMessage(from, `• Hasil: ${anu.result.hasil}\n• Dibuat Pada: ${anu.result.create_at}`, text, {quoted: freply})
					break
				case 'shorturl':
...
`
        ...
        break;
				...
				for (let i = 0; i < argsi[1]; i++){
					Putra.sendMessage(from, argsi[0], MessageType.text)
				}
				break
			...
			case 'delrespon':
				...
				case 'firework':
					...
					if ((isMedia && !kyy.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						Putra.sendMessage(from, buff, video, {quoted: freply, caption: mess.success})
					} else {
			...
			case 'blur':
              ...
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc', imageMessage: imageMsg,
              contentText:`Follow Ig @putragans167`,buttons,headerType:4}
              prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Putra.relayWAMessage(prep)
...
anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/ppcouple?apikey=IkyOgiwara`)
              ...
              imageMsg = (await Putra.prepareMessageMedia(buff1, "imageMessage", { thumbnail: buff1, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc', imageMessage: imageMsg,
            ...
            buff2 = await getBuffer(anu.result.female)
              buttons = [{buttonId: `!infoig`,buttonText:{displayText: `INSTAGRAM`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff2, "imageMessage", { thumbnail: buff2, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc', imageMessage: imageMsg,
              ...
              Putra.relayWAMessage(prep)
                ...
                reply (mess.wait)
                  ...
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${setting.lolkey}&text=${ini_txt}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'created By Created By Putraxd Ofc', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow Ig @putragans167`,buttons,headerType:4}
              prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Putra.relayWAMessage(prep)
                    break
                ...
                reply (mess.wait)
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=&text1=${txt1}&text2=${txt2}`)
                          buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow Ig @putragans167`,buttons,headerType:4}
             ...
             buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/${command}?apikey=IkyOgiwara`)
              ...
              Putra.relayWAMessage(prep)
                   ...
                   case 'setwelcome':
					...
					join = body.slice(11)
					Putra.sendMessage(from ,`\`\`\`Welcome berhasil di ubah menjadi : ${body.slice(11)}\`\`\``, text,{quoted : freply})
				break 
...
case "mode":
	      ...
	      case 'self':
              ...
              break
                ...
                case 'quotesimage':
                    ...
                    buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'PUTRA BOT', imageMessage: imageMsg,
       ...
       // Movie & Story
                case 'lk21':
                    ...
                    await Putra.sendMessage(from, thumbnail, image, { quoted: freply, caption: ini_txt })
                    break
                case 'drakorongoing':
...
ׄ✑⋆ ⃝⃕⋆⬡  ⃝⃕⋆⬡  ⃝⃕⋆⬡⋆ ⃝⃕⋆⬡  ⃝⃕⋆⬡ ⃝⃕ׄ
 ...
 ׄ𝔍𝔞𝔫𝔤𝔞𝔫 𝔭𝔢𝔯𝔫𝔞𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔰𝔢𝔰𝔢𝔬𝔯𝔞𝔫𝔤 
  𝔶𝔞𝔫𝔤 𝔪𝔞𝔰𝔦𝔥 𝔪𝔢𝔫𝔠𝔦𝔫𝔱𝔞𝔦 𝔪𝔞𝔰𝔞𝔩𝔞𝔩𝔲𝔫𝔶𝔞
`
    ...
    {buttonId: `${prefix}rules`, buttonText: {displayText: 'S&K'}, type: 1},
               ...
               imageMsg = (await Putra.prepareMessageMedia(fs.readFileSync(`./media/Menu.jpg`), 'imageMessage', { thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true }})).imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText:  `   

 Created By Putraxd Ofc︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}


               prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
                          Putra.relayWAMessage(prep)
               break
        case 'storymenu':
        case 'moviemenu':
...
┃▹ addupdate *fiturnya*
┃▹ exif *nama|author*
┃▹ sewa add/del *waktunya*
┃▹ premium add @tag|nomor
┃▹ premium del @tag|nomor
┗━▹
...
き⃟🦈 𝐆𝐫𝐨𝐮𝐩 𝐂𝐡𝐚𝐭𝐬 : *${groups.length}*
き⃟🦈  𝐏𝐫𝐢𝐯𝐚𝐭𝐞 𝐂𝐡𝐚𝐭𝐬 : *${privat.length}*
き⃟🦈 𝐓𝐨𝐭𝐚𝐥 𝐂𝐡𝐚𝐭𝐬 : *${totalChat.length}*
き⃟🦈 𝐒𝐩𝐞𝐞𝐝 : *${latensie.toFixed(4)} _Second_*
き⃟🦈 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦 : *${os.platform()}*
...
`
Putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'Created By PutraXd Ofc', buttons: [{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🏷️ BACK TO MENU' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
   //addfiturbokep
                case 'bokep1':				 
				...
				qute = fs.readFileSync('media/Menu.jpg') 
				   ...
				   Putra.sendMessage(from, qute, image, { quoted: freply, caption: '*SEMOGA DI BERI HIDAYAH*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
				   break
				   case 'bokep2':
   ...
   case 'nekopoi1':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www2.zippyshare.com/d/z9dcY6Nr/791216/%5bNekoPoi%5d_Akina_to_Onsen_de_H_Shiyo%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi2':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www4.zippyshare.com/d/j7PFLGUY/946757/%5bNekoPoi%5d_Tsuma_ga_Kirei_ni_Natta_Wake_-_02_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi3':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www111.zippyshare.com/d/EK5uqIMh/566060/%5bNekoPoi%5d_Megane_no_Megami_-_01_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi4':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www109.zippyshare.com/d/2ebALhhS/463654/%5bNekoPoi%5d_Watashi_ga_Toriko_-_02%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi5':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www115.zippyshare.com/d/LmzOkRAI/313299/%5bNekoPoi%5d_Chichi-iro_Toiki_-_01_%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi6':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www89.zippyshare.com/d/SgEHvrJs/42812/%5bNekoPoi%5d_Chichi-iro_Toiki_-_02_%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi7':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www66.zippyshare.com/d/j0ivbciL/520411/%5bNekoPoi%5d_Onna_Maou_Melissa_no_H_na_Boukenki_-_01_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi8':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www71.zippyshare.com/d/M225YIrR/535656/%5bNekoPoi%5d_Buta_no_Gotoki_Sanzoku_ni_Torawarete_Shojo_wo_Ubawareru_Kyonyuu_Himekishi_Onna_Senshi_-_01%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi9':
qute = fs.readFileSync('media/Menu.jpg') 
Putra.sendMessage(from, qute, image, { quoted: freply, caption: 'NIH LINKNYA: https://www49.zippyshare.com/d/bdwYjaXS/605790/%5bNekoPoi%5d_Akebi_no_Hana___Maho_-_01_%5b360P%5d_%5bnekopoi.pro%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break        
...
`
 ...
 Created By Putraxd Ofc︎`, imageMessage: imageMsg,
      ...
      case 'command':
               list = []
               listmenu = [`allmenu`,`runtime`,`status`,`groupmenu`,`nekopoimenu`,`dosamenu`,`photoxy`,`makermenu`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`gcmikel`,`sewabot`,`putraxd`,`info`,`patner`,]
               listmenuu = [`All Menu Bot`,`Runtime`,`Status Bot`,`Menu Group`,`Nekopoi Menu`,`Dosa Menu`,`Photo Oky`,`Image Maker`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`18+ Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`,`Creator Script Bot`,`Thanks To`,`Patner Bot`,]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: 'menu ke' + nombor++,
...
`,`🐣 Day : ${week} ${weton}\n🐥 Date : ${date}              `, list, { quoted: freply})
               ...
               case 'gcmikel':
               reply("https://chat.whatsapp.com/FYcSmBK4qVW3yvna8Yyzu3")
               break
...
// Islami //
                ...
                case 'alquranaudio':
                    ...
                    await Putra.sendMessage(from, ini_buffer, audio, { quoted: freply, mimetype: Mimetype.mp4Audio })
                    break
                case 'asmaulhusna':
                ...
                case 'jadwalsholat':
                    ...
                    break
      case 'rules':
             Putra.sendMessage(from, rulesBot(prefix), MessageType.text, {quoted: freply})
             break
               ...
               case 'slow':
					...
					media = await Putra.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						Putra.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: freply})
						fs.unlinkSync(ran)
					})
					break
				case 'stickerlist':
...
break
			...
			case 'gets':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list sticker')
				...
				Putra.sendMessage(from, result, sticker)
				break
           case 'liststicker':
				teks = '*Sticker List :*\n\n'
				for (let awokwkwk of setiker) {
			...
			case 'getvideo':
			   if (args.length < 1) return reply('Masukan nama yang terdaftar di list video')
				...
				Putra.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_SelfBot OnlyDev City_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JStw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} }} }) 
				break
			case 'listvideo':
			case 'videolist':
				teks = '*List Video :*\n\n'
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               Putra.sendMessage(from, starGame, text, {quoted: freply, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated Created By Putraxd Ofc︎`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
       ...
       case 'tebakanime':
              ...
              Putra.sendMessage(from, ini_buffer, image, { quoted: freply, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
       ...
       case 'tebaklagu':
              ...
              reply('*+* ```Tebak Lagu```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s')
              Putra.sendMessage(from, ini_buffer, audio, {quoted: freply}).then(() => {
       ...
       case 'kuismath':
              ...
              jawaban = get_result.jawaban
              ini_buffer = await getBuffer(ini_image)
              Putra.sendMessage(from, ini_buffer, image, { quoted: freply, caption: '*+* ```Kuis Matematika```\n\nSilahkan jawab soal berikut ini\n\n• *Waktu* : 50s' }).then(() => {
       ...
       case 'tebakgambar':
            ...
            Putra.sendMessage(from, buff, image, { quoted: freply, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+kisi_kisi+'\nWaktu : 30s' }).then(() => {
       ...
       case 'siapaaku':
              ...
              Putra.sendMessage(from, '*+* ```Tebak Siapakah Aku```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'tebakkata':
              ...
              Putra.sendMessage(from, '*+* ```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'tebaklirik':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Putra.sendMessage(from, '*+* ```Tebak Lirik```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
      ...
      case 'tebakjenaka':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Putra.sendMessage(from, '*+* ```Tebak Jenaka```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'kimiakuis':
              ...
              Putra.sendMessage(from, '*+* ```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'tebakbendera':
              ...
              Putra.sendMessage(from, '*+* ```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'susunkata':
              ...
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              Putra.sendMessage(from, '*+* ```Susun Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: freply}).then(() => {
       ...
       case 'asahotak':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.pertanyaan
              Putra.sendMessage(from, '*+* ```Asah Otak```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
       ...
       case 'caklontong':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Putra.sendMessage(from, '*+* ```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: freply}).then(() => {
...
const serialUser = createSerial(18)
	         try {
								ppimg = await Putra.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*「 Putra Bot 」*`
         ikyads = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=https://bit.ly/walpamikel`)
             buttons = [{buttonId: `!menu`,buttonText:{displayText: `🏷️MENU`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(ikyads, "imageMessage", { thumbnail: ikyads, })).imageMessage
	    ...
	    // console.log(e)
            setTimeout( () => {
			Putra.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di Putra Botき⃟🦈*`)
		}, 2000)
        break
case 'sfire':
{
                ...
                if (isImage || isQuotedImage) {
                    ...
                    let yoooo = await Putra.downloadAndSaveMediaMessage(encmedia)
...
//------------------< Premium >-------------------
       ...
       case 'sewabot':
              gopeynya = 'https://telegra.ph/file/4b1395ceb05db995fbbea.jpg'
...
`
  buttons = [{buttonId: `${prefix}buypremium`,buttonText:{displayText: `Premium`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT`},type:1}]
              imageMsg = (await Putra.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Created By Putraxd Ofc︎ ', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await Putra.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              Putra.relayWAMessage(prep)
      break
case 'tod':
              gopeynya = 'https://telegra.ph/file/7f3047e94d431d40df7ce.jpg'
            buff = await getBuffer(gopeynya)
            teksnya = `
...
┏━━⬣ MINAT? PM
┃⬡ wa.me/6283842942635
┃⬡ Group : https://bit.ly/3eqhu5c
┃⬡ Github Putragans9
┗━━⬣

`
Putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'Created By PutraXd Ofc', buttons: [{ buttonId: `!payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 },{ buttonId: `!store`, buttonText: { displayText: '🏷️ BACK TO MENU' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
      case 'payment':
      case 'pay':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               Putra.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0), caption: vars, quoted : kyy})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
if (err) return reply(`${err}`)
Putra.sendMessage(from,{url:'./'+kyyyy},audio,{mimetype:'audio/mpeg'})
})
       ...
       case 'tiktoknowm':
              ...
              imageMsg = ( await Putra.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
  ...
  case 'xnxxsearch':
              ...
              /////////////////////////////////////////////////////////////////////////////////////////////////////////////////if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://Putra-chan02.github.io`)
      ...
      case 'tiktokaudio':
             ...
             Putra.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: freply })
             break
      case 'fbdl':
      case 'fb':
      case 'facebok':
					...
					Putra.sendMessage(from, buff, image, {quoted: freply, caption: teks})
					break
      case 'twitter':
      ...
      case 'ssweb':
             ...
             await Putra.sendMessage(from, ini_buffer, image, { quoted: freply })
             break
       case 'nhentaipdf':
...
🐋 Character : ${get_info.characters.join(", ")}\n`
             ...
             Putra.sendMessage(from, ini_image, image, { caption: teks, quoted: freply, thumbnail: Buffer.alloc(0) })
             reply(mess.wait)
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=${setting.lolkey}`)
             pdf = await getBuffer(anu.result)
             Putra.sendMessage(from, pdf, document, { quoted: freply, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
       ...
       case 'doujindesu':
             ...
             Putra.sendMessage(from, buffer, image, {caption: xixixai, quoted: freply})
             } catch (e) {
             console.log(e)
             reply(String(e))
}
      ...
      case 'kusonime':
             ...
             await Putra.sendMessage(from, ini_buffer, image, { quoted: freply, caption: ini_txt })
             break
       case 'otakudesu':
              if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
              reply(mess.wait)
       ...
       case 'storyanime':
              ...
              Putra.sendMessage(from, buffer, video, { quoted: freply })
              break
       case 'neko3d':
       case 'nekopoi3d':
       case '3dnekopoi':
...
*[Wait] Tunggu Sebentar..*`
				...
				Putra.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Nih Gan'})
					break  
...
imageMessage = (await Putra.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage
...
\`\`\`🦈 Link : ${get_data[i].url}\`\`\``
              ...
              Putra.sendMessage(from, ini_buffer, image, { quoted: freply, caption: teks })
              } catch {
       ...
       case 'playstore':
              ...
              query = args.join(" ")
              reply(mess.wait)
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/apkdownloader?apikey=${setting.lolkey}&package=${query}`)
              get_result = get_result.result
              teks = `┏┉⌣ ┈̥-̶̯͡..̷̴✽̶┄┈┈┈┈┈┈┈┈┈┈┉┓
┆ *PLAYSTORE*
└┈┈┈┈┈┈┈┈┈┈┈⌣ ┈̥-̶̯͡..̷̴✽̶⌣ ✽̶
...
\`\`\`🎣 Link : ${i.url}\`\`\``
       ...
       case 'tourl':
               ...
               owgi = await Putra.downloadMediaMessage(boij)
       ...
       case 'img2url':
               ...
               var media = await  Putra.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               Putra.sendMessage(from, ibb, image, { quoted: freply, caption: caps})
})
              .catch(err => {
               throw err
})
        ...
        case 'tulis':
               ...
               Putra.sendMessage(from, buff, image, {quoted: freply, caption: mess.success}).catch(e => {
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await Putra.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              ...
              Putra.sendMessage(from, buffer, image, { caption: teks, quoted: freply})
              break
...
//------------------< Stalk >-------------------
      case 'stalkgithub':
      case 'githubstalk':
              if (args.length == 0) return reply(`Example: ${prefix + command} Putra-chan02`)
              reply(mess.wait)
              username = args[0]
...
\`\`\`🦈 Link : ${ini_result.url}\`\`\`
`
             Putra.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} Putra.chan26`)
             reply(mess.wait)
             username = args[0]
...
`
              Putra.sendMessage(from, pp_tt, image, { quoted: freply, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
       case 'iguser':
              try {
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'telestiker':
              ...
              Putra.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'semoji':
       case 'emoji':
              ...
              await Putra.sendMessage(from, ini_buffer, sticker, { quoted: freply })
              break
case 'ttp':
...
break
       case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Putra`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              Putra.sendMessage(from, buffer, sticker, { quoted: freply })
              break
              case 'tg': 
...
break
       ...
       case 'triggered':
                ...
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : kyy
                    let media = await Putra.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    Putra.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
           ...
           teks = args.join(' ')
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/ttg.webp -o ./sticker/ttg.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    Putra.sendSticker(from, fs.readFileSync(`./sticker/ttg.webp`), sticker, {quoted: freply})
                    limitAdd(sender, limit)
       ...
       case 'gifstiker':
            ...
            const media = await Putra.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                .on('start', function (cmd) {
                ...
                .on('end', function () {
                console.log('Finish')
                Putra.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: freply})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
                .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
       ...
       case 'colong':
              ...
              require('./lib/fetch.js').modStick(media, Putra, kyy, from)
              break
       case 'delwm':
              if (!isQuotedSticker) return reply('Stiker aja om')
              encmedia = JSON.parse(JSON.stringify(kyy).replace('quotedM','m')).message.extendedTextMessage.contextInfo
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              Putra.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: freply })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              Putra.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: freply })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
      ...
      case 'toimg':
              ...
              Putra.sendMessage(from, buffer, image, {quoted: freply, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
       case 'smeme': 
...
break
       ...
       case 'memegen':                    
        ...
        case 'togif':
               ...
               mediaaa = await Putra.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Putra.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: freply, caption: mess.success})
        ...
        case 'tovideo':
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Putra.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: freply, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               reply(mess.wrongFormat)
}
...
buffer = fs.readFileSync(ran)
Putra.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4'})
fs.unlinkSync(ran)
})
break
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1}]
       ...
       case 'sc': 
       case 'src':
              reply("Bot Ini Menggunakan Script :\nhttps://github.com/Putragans9/BOT4")
              break
...
//------------------< Owner >-------------------
      ...
      case 'join': 
             ...
             fak = Putra.query({ json: ['action', 'invite', link],
             expect200: true })
      ...
      case 'broadcast':
             ...
             anu = await Putra.chats.all()
             if (isMedia && !kyy.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM','m')).message.extendedTextMessage.contextInfo : kyy
             bc = await Putra.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Putra.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 𝑠𝑢𝑏𝑠𝑐𝑟𝑖𝑏𝑒 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 Putra Bot Broadcast 」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
      ...
      case 'broadcast2':
             ...
             for (let _ of anu) {
Putra.sendMessage(_.jid, 
			{"contentText": `*「 Putra Bot Broadcast 」*\n\n${body.slice(4)}`,
			"footerText": 'Putra Bot㌹',
			"buttons": [
			{"buttonId": `#menu`,
			"buttonText": {"displayText": "MENU BOT"
             ...
             if (args.length < 1) return reply('teks?')
             anu100 = await Putra.chats.all()
             if (isMedia && !Putra.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Putra).replace('quotedM','m')).message.extendedTextMessage.contextInfo : Putra
             bc100 = await Putra.downloadMediaMessage(encmedia)
             for (let _ of anu100) {
             Putra.sendMessage(_.jid, bc100, image, {quoted: freply, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu100) {
      ...
      case 'clearall':
             if (!isOwner) return  reply(mess.only.owner)
             anu = await Putra.chats.all()
             Putra.setMaxListeners(25)
             for (let _ of anu) {
             Putra.deleteChat(_.jid)
}
      ...
      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = Putra.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
...
//------------------< G R U P >-------------------
      ...
      case 'promote':
             ...
             Putra.groupMakeAdmin(from, mems_ids)
             } else {
             Putra.groupMakeAdmin(from, entah)
}
             } else {
             entah = kyy.message.extendedTextMessage.contextInfo.participant
             Putra.groupMakeAdmin(from, [entah])
      ...
      case 'demote':
             ...
             Putra.groupDemoteAdmin(from, mems_ids)
             } else {
             Putra.groupDemoteAdmin(from, [entah[0]])
}
             } else {
             entah = kyy.message.extendedTextMessage.contextInfo.participant
             Putra.groupDemoteAdmin(from, [entah])
}
             break
       case 'setgrupname':
              if (!isGroup) return reply(mess.only.group)
              ...
              Putra.groupUpdateSubject(from, q)
       ...
       case 'setdesc':
              ...
              Putra.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
              ...
              if (isQuotedImage) {
              let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(kyy).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : kyy
              let media = await Putra.downloadMediaMessage(encmedia)
              Putra.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
       ...
       case 'profile':
              ...
              Putra.updatePresence(from, Presence.composing)
              try {
              profil = await Putra.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
              } catch {
              profil = errorImg
}
              thu = await Putra.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
              ...
              Putra.sendMessage(from, buff, image, {quoted: freply, caption: profile})
              break
       case 'afk': 
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'infogc':
              if (!isGroup) return reply(mess.only.group)
              try {
              var pic = await Putra.getProfilePicture(from)
              } catch {
              var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
              let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Descripti:* \n${groupMetadata.desc}`
              Putra.sendMessage(from, await getBuffer(pic), image, {quoted: freply, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
              break
       case 'tagall':
              if (!isGroup) return reply(mess.only.group)
              if (!isGroupAdmins) return reply(mess.only.admin)
       ...
       case 'here':                
             ...
             let online = [...Object.keys(Putra.chats.get(ido).presences), Putra.user.jid]
             Putra.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: freply, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      ...
      case 'sider':
             if(!isGroup) return reply(mess.only.group)
             try {
             infom = await Putra.messageInfo(from, kyy.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Dibaca oleh:*\n\n`
...
//------------------< Fun >-------------------
       ...
       case 'toxic':
        ...
        case 'citacita':
              ...
              Putra.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: freply})
              break
       case 'apakah':
              apakah = body.slice(1)
              const apa =['Iya','Tidak','Bisa Jadi','Hmm','Coba Ulangi']
              const kah = apa[Math.floor(Math.random() * apa.length)]
              Putra.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: freply })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              Putra.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: freply })
              break
      case 'bay':
      ...
      break
       ...
       case 'cekganteng':
              ...
              Putra.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: freply })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              Putra.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: freply })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await Putra.getProfilePicture(sender)
       ...
       case 'truth':
              ...
              Putra.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: freply })
              break
       case 'dare':
...
//------------------< Lainnya >-------------------
        case 'getpp':
               if (kyy.message.extendedTextMessage === null || kyy.message.extendedTextMessage === undefined) {
               linkpp = await Putra.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               Putra.sendMessage(from, buffer, image, {caption: "Nih", quoted: freply })
               ...
               linkpp = await Putra.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               Putra.sendMessage(from, buffer, image, { quoted: freply, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
        ...
        case 'delete': // MR.CYSER
               ...
               Putra.deleteMessage(from, {id: kyy.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true})
               } catch (e){
               reply('Reply chat bot')
}
               break
...
━━━━━━━ •♬• ━━━━━━━
`
Putra.sendMessage(from, { contentText: `${menuny}`, footerText: 'Created By PutraXd Ofc', buttons: [{ buttonId: `!menu`, buttonText: { displayText: 'BACK TO MENU' }, type: 1 } ],  headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break 
...
case 'caripesan':  //by ANU TEAM
             ...
             reply(mess.wait)
             cok = await Putra.searchMessages(`${ve}`, from, batas,1) 
             if (cok.messages.length < 2) return reply('Tidak Ditemukan Pesan') 
             if (cok.messages.length < parseInt(batas)) reply(`Hanya Ditemukan ${cok.messages.length - 1} Pesan`)
             for (i=1;i < cok.messages.length;i++) {
             if (cok.messages[i].message) {
             Putra.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
       ...
       case 'bugreport': 
              ...
              Putra.sendMessage('6283842942635@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              Putra.sendMessage('6283842942635@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT RAVA DONE')
              Putra.sendMessage('6283842942635@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
              totalchat.map( async ({ jid }) => {
              await Putra.chatRead(jid)
})
...
//------------------< Enable/Disable >-------------------
      ...
      case 'grup':
             ...
             if (args[0].toLowerCase() === 'enable'){
             Putra.groupSettingChange(from, "announcement", false)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
             } else if (args[0].toLowerCase() === 'disable'){
             Putra.groupSettingChange(from, "announcement", true)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://instagram.com/putragans167`)
             break
      case 'infoyt':
            reply(`Jangan Lupa Subscribe YouTube Owner Ya : https://youtube.com/channel/UCJGX0lPZWVDidVL2pEFDaSw`)
            break
      case 'hamdeh':
             reply('Hamdeh hamdeh')
             break
...
if (budy.includes(`anjing`)) {
		...
		if (budy.includes(`Assalamualaikum`)) {
                  ...
                  const loli = fs.readFileSync('./assets/robot.mp3')
...
• Pemenangnya adalah [ @${winR} ] `
	   Putra.sendMessage(from, starGame, text, {quoted: freply, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./media/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		Putra.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  Putra.sendMessage(from, ucapan, text, {quoted: freply, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
Putra.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:kyy,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
Putra.sendMessage(from, ucapan1, text, {quoted: freply, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
Putra.sendMessage(from, ucapan1, text, {quoted:kyy, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
